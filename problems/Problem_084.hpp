...
int num_islands( std::vector<std::vector<int>> island_map )
{
  ...
  for( int i = 0; i < island_map.size(); i++ )
  {
    for( int j = 0; j < island_map[0].size(); j++ )
    {
      ...
      {    // Current "tile" is land
        if( j - 1 >= 0 )
          if( visited[i][j - 1] && island_map[i][j - 1] ) continue;    // Up
        if( i - 1 >= 0 )
          if( visited[i - 1][j] && island_map[i - 1][j] ) continue;    // Left
        if( j + 1 < island_map[0].size() )
          if( visited[i][j + 1] && island_map[i][j + 1] ) continue;    // Down
        if( i + 1 < island_map.size() )
          if( visited[i + 1][j] && island_map[i + 1][j] ) continue;    // Right
        if( j - 1 >= 0 && i - 1 >= 0 )
          if( visited[i - 1][j - 1] && island_map[i - 1][j - 1] ) continue;    // Up Left
        if( j - 1 >= 0 && i + 1 < island_map.size() )
          if( visited[i + 1][j - 1] && island_map[i + 1][j - 1] ) continue;    // Up Right
        if( j + 1 < island_map[0].size() && i - 1 >= 0 )
          if( visited[i - 1][j + 1] && island_map[i - 1][j + 1] ) continue;    // Down Left
        if( j + 1 < island_map[0].size() && i + 1 < island_map.size() )
          if( visited[i + 1][j + 1] && island_map[i + 1][j + 1] ) continue;    // Down Right
