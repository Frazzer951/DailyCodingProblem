...
def get_problems() -> Dict[str, List[str]]:
    ...
    path = os.path.join(ROOT_DIR, "../problems/include")
...
def create_readme_links(files: List[str]) -> List[str]:
    ...
    for file, difficulty in files:
        num = int(file.rstrip(".hpp").split("_")[-1])
        section = (num - 1) // 10
        text.append(
            f" - [Problem {num:03}](problems/include/problems_{section:02}1_{section + 1:02}0/{file}) - {difficulty}")
