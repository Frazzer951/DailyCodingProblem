...
template<typename T> struct slNode {
  ...
  slNode( std::vector<T> l ) : value( l[0] ) {
    if ( l.size() > 1 )
      next = new slNode( std::vector<T>( l.begin() + 1, l.end() ) );
    else
      next = nullptr;
  }
...
template<typename T> inline bool operator==( const slNode<T> &lhs, const slNode<T> &rhs ) {
...
template<typename T> inline std::ostream &operator<<( std::ostream &os, const slNode<T> &n ) {
  os << n.value;
...
template<typename T> inline bool operator==( const slList<T> &lhs, const slList<T> &rhs ) {
...
template<typename T> inline std::ostream &operator<<( std::ostream &os, const slList<T> &n ) {
  os << n.head;
  return os;
}
#endif
